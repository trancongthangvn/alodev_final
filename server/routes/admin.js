...
// Fail-fast: refuse to boot with default creds (audit fix May 2026).
const ADMIN_USER = process.env.ADMIN_USER
const ADMIN_PASS = process.env.ADMIN_PASS
if (!ADMIN_USER || !ADMIN_PASS || ADMIN_USER.length < 3 || ADMIN_PASS.length < 16) {
  console.error("FATAL: ADMIN_USER (>=3 chars) and ADMIN_PASS (>=16 chars) must be set in server/.env")
  process.exit(1)
}
...
router.use((req, res, next) => {
  ...
  try {
    ...
    if (!timingSafeEq(user, ADMIN_USER) || !timingSafeEq(pass, ADMIN_PASS)) throw new Error('bad creds')
    next()
  } catch {
...
router.get('/leads', (req, res) => {
  ...
  res.json({ results: leads, total, limit, offset })
})
...
router.get('/blog', (req, res) => {
  ...
  res.json({ results: posts, total, limit, offset })
})
