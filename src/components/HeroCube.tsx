...
function setupCube(
  ...
  // Cubic ease-in-out - used for ALL twists now (May 2026 retire of
  // easeOutQuint). Smooth start, smooth landing - the wave reads as
  // flowing rather than punchy. Resend / Linear cube animations use
  // exactly this curve family.
  ...
  // Slow showcase flip loop (May 2026 - "mượt như resend.com").
  // Replaced the previous "human speedcuber burst" cadence (which felt
  // rushed at 220-340ms quarter turns + 60-130ms gaps) with the
  // contemplative register from resend.com / linear.app: each twist
  // takes ~3x longer, all use the same smooth ease-in-out, gaps are
  // ~3-5x longer. Reads as a "showcase rotation" rather than a
  // "speedcuber recording".
  //
  //   - 92% quarter turns / 8% double turns (kept - gives variety)
  //   - Quarter: 750-1000ms with ease-in-out-cubic (was 220-340 + quintic)
  //   - Double: 1100-1400ms with ease-in-out-cubic
  //   - Burst length: 1-3 moves (was 3-6) - ripple, not flurry
  //   - Inner gap: 250-450ms - visible breath between moves
  //   - Think pause: 1800-3500ms - confidence in stillness
  //   - Wrapper-snap reorient unchanged (~25% between bursts)
  async function flipLoop() {
    const initialHold = opts.reducedMotion ? 2800 : 2000
    await new Promise((r) => { timer1 = setTimeout(r, initialHold) })

    let lastAxis: 'x' | 'y' | 'z' | null = null

    while (!stopped) {
      // Burst length: 1-3 moves (was 3-6)
      ...
      for (let i = 0; i < burstLen && !stopped; i++) {
        ...
        // Quarter: 750-1000ms (was 220-340). Double: 1100-1400ms (was 360-500).
        ...
        // All twists use the SAME smooth ease-in-out - no more snappy
        // quintic flick. Reads as deliberate, not reactive.
        await performTwist({
        ...
        // Inter-move gap inside a burst: 250-450ms (was 60-130ms)
        const innerGap = opts.reducedMotion ? 900 : 250 + Math.random() * 200
        await new Promise((r) => { timer2 = setTimeout(r, innerGap) })
      }

      if (stopped) break

      // Between-burst "thinking" pause: 1800-3500ms (was 600-1500ms)
  ...
  // any single axis.
  // Speed reduced May 2026 (founder feedback "giảm tốc độ xoay"): 0.005
  // → 0.003 (~28s per full rotation, was ~17s). Cube reads more
  // contemplative - closer to a slow turntable than a spinning top.
  const TUMBLE = opts.reducedMotion ? 0.0015 : 0.003
  ...
  const REVOLUTIONS_PER_PAGE = opts.reducedMotion ? 0 : 1.5
