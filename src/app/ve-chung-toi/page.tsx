...
const toc = [
  { num: '01', name: 'Hồ sơ',     hash: '#hero' },
  { num: '02', name: 'Số liệu',   hash: '#stats' },
  { num: '03', name: 'Thông tin', hash: '#thong-tin-chung' },
  { num: '04', name: 'Nguyên tắc', hash: '#nguyen-tac' },
  { num: '05', name: 'Lĩnh vực', hash: '#linh-vuc' },
  { num: '06', name: 'Stack',     hash: '#stack' },
  { num: '07', name: 'Cam kết',   hash: '#cam-ket' },
  { num: '08', name: 'Hành trình', hash: '#hanh-trinh' },
  { num: '09', name: 'Founder',   hash: '#founder' },
  { num: '10', name: 'Liên hệ',   hash: '#cta' },
]
...
export default function VeChungToiPage() {
  return (
    <MagazineLayout toc={toc} tagline={<>Một founder,<br />một studio.</>}>
      <JsonLd data={[
        breadcrumbSchema([
          { name: 'Trang chủ', url: '/' },
      ...
      <section id="hero" className="mag-section mag-bg-paper relative overflow-hidden">
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 lg:py-24">
          <p className="mag-section-index">Hồ sơ năng lực</p>
          <h1 className="mag-display !mt-3 max-w-3xl">
            Một founder, một studio,<br />
            <span>11+ sản phẩm vận hành.</span>
          </h1>
          <p className="mag-body-lead mt-4 max-w-2xl">
            Founder-led studio Hà Nội - tự phát triển + vận hành 11+ sản phẩm thật trên domain riêng.
            Bạn ký với người đang code và operate, không qua sales/PM.
          </p>
      ...
      {/* STATS ─────────────────────────────────────────────────────────── */}
      <section id="stats" className="mag-section mag-bg-tint py-12 lg:py-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-px bg-gray-200 dark:bg-ink-800 rounded-2xl border border-gray-200 dark:border-ink-800 overflow-hidden">
            {stats.map((s) => (
              <div key={s.label} className="bg-white dark:bg-ink-950 px-5 py-7 text-center">
                <div className="tabular text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">
                  {s.num}<span className="text-sm font-medium text-gray-500 dark:text-ink-500 ml-1">{s.unit}</span>
                </div>
                <div className="text-xs sm:text-sm text-gray-600 dark:text-ink-400 mt-2">{s.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="thong-tin-chung" className="mag-section mag-bg-paper py-10 lg:py-24">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="reveal grid grid-cols-1 sm:grid-cols-2 gap-px bg-gray-200 dark:bg-ink-800 rounded-2xl border border-gray-200 dark:border-ink-800 overflow-hidden">
            {identityRows.map((row) => (
              <div key={row.label} className="bg-white dark:bg-ink-950 px-5 py-4">
                <div className="text-xs font-semibold uppercase tracking-widest text-gray-500 dark:text-ink-500 font-mono">{row.label}</div>
      ...
      <section id="nguyen-tac" className="mag-section mag-bg-tint py-10 lg:py-24">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="reveal max-w-2xl">
            <p className="mag-section-index">02 · Nguyên tắc</p>
            <h2 className="mag-section-head !mt-3">Bốn nguyên tắc làm việc.</h2>
          ...
          <div className="reveal-stagger mt-12 grid grid-cols-1 md:grid-cols-2 gap-5">
            {principles.map((p) => (
              <div key={p.title} className="lift spotlight rounded-2xl border border-ink-100 dark:border-ink-800 bg-white dark:bg-ink-900 p-7">
                ...
                <h3 className="mt-4 mag-card-head text-ink-900 dark:text-white">{p.title}</h3>
      ...
      <section id="linh-vuc" className="mag-section mag-bg-paper py-10 lg:py-24">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">

          {/* - Services - */}
          <div className="reveal max-w-2xl">
            <p className="mag-section-index">03 · Lĩnh vực hoạt động</p>
            <h2 className="mag-section-head !mt-3">Phạm vi dịch vụ Alodev triển khai.</h2>
            <p className="mt-4 text-gray-600 dark:text-ink-400 leading-relaxed">
          ...
          <div className="reveal-stagger mt-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {services.map((s) => (
              ...
              >
                ...
                <h3 className="mt-4 text-sm sm:text-base font-bold text-ink-900 dark:text-white group-hover:text-brand-700 dark:group-hover:text-brand-400 transition">{s.title}</h3>
          ...
          <div className="reveal max-w-2xl mt-16 lg:mt-24">
            <p className="mag-section-index">04 · Ngành phục vụ</p>
            <h2 className="mag-section-head !mt-3">Sáu ngành Alodev đã triển khai sản phẩm vận hành.</h2>
            <p className="mt-4 text-gray-600 dark:text-ink-400 leading-relaxed">
          ...
          <div className="reveal-stagger mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {industries.map((ind) => (
              <div key={ind.name} className="rounded-2xl border border-ink-100 dark:border-ink-800 bg-cream-50 dark:bg-ink-900 p-5">
                ...
                <h3 className="mt-2 text-sm sm:text-base font-bold text-ink-900 dark:text-white">{ind.name}</h3>
      ...
      <section id="stack" className="mag-section mag-bg-tint py-10 lg:py-24">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="reveal max-w-2xl">
          ...
          <div className="reveal-stagger mt-10 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
            {stackGroups.map((g) => (
              <div key={g.label} className="rounded-xl border border-gray-200 dark:border-ink-800 bg-white dark:bg-ink-900 p-5">
                <div className="text-xs font-semibold uppercase tracking-[0.18em] text-gray-500 dark:text-ink-500 mb-3">{g.label}</div>
      ...
      {/* CAM KẾT CHẤT LƯỢNG (NEW) - concrete SLA-style commitments that
          ...
          "trễ → -5%/tuần") pass it. */}
      <section id="cam-ket" className="mag-section mag-bg-paper py-10 lg:py-24">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="reveal max-w-2xl">
            <p className="mag-section-index">06 · Cam kết chất lượng</p>
            <h2 className="mag-section-head !mt-3">Bảy cam kết được ghi vào hợp đồng.</h2>
            <p className="mt-4 text-gray-600 dark:text-ink-400 leading-relaxed">
              Đo được và ghi vào hợp đồng.
            </p>
          </div>
      ...
      <section id="hanh-trinh" className="mag-section mag-bg-tint py-10 lg:py-24">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="reveal max-w-xl">
            <p className="mag-section-index">07 · Hành trình</p>
            <h2 className="mag-section-head !mt-3">Năm năm fleet operation.</h2>
          </div>
          <div className="reveal-stagger mt-14 relative">
            ...
            {milestones.map((m) => (
              <div key={m.year} className="relative pl-20 sm:pl-24 pb-8 last:pb-0">
                ...
                <div className="absolute left-[2.5rem] sm:left-[3.25rem] top-2 w-2.5 h-2.5 rounded-full bg-brand-600 dark:bg-brand-400 ring-4 ring-slate-50 dark:ring-ink-950" />
      ...
      <section id="founder" className="scroll-mt-20 mag-section mag-bg-paper py-10 lg:py-24">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="reveal rounded-3xl border border-ink-100 dark:border-ink-800 bg-cream-50 dark:bg-ink-900 p-8 lg:p-10">
            ...
            <h2 className="mt-5 mag-card-head text-ink-900 dark:text-white">Đội ngũ - minh bạch về quy mô</h2>
            <div className="mt-4 space-y-4 text-gray-700 dark:text-ink-300 leading-relaxed">
              ...
              <p>
                Mỗi quý nhận 3–5 dự án. Cần đội 20 dev enterprise 12 tháng → Alodev không hợp. Cần sản phẩm bàn giao đúng hạn, code đọc được → hợp.
              </p>
              <p>
      ...
      {/* HỒ SƠ DỰ ÁN TIÊU BIỂU (NEW) - featured 6 projects spanning all
          ...
          stay in sync. */}
      <section className="mag-section mag-bg-tint py-10 lg:py-24">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="reveal flex items-end justify-between flex-wrap gap-4 mb-8 lg:mb-12">
            <div className="max-w-2xl">
              <p className="mag-section-index">08 · Hồ sơ dự án tiêu biểu</p>
              <h2 className="mag-section-head !mt-3">Sáu sản phẩm đại diện cho sáu ngành.</h2>
              <p className="mt-4 text-gray-600 dark:text-ink-400 leading-relaxed">
              ...
              >
                <div className={`relative aspect-[16/10] bg-gradient-to-br ${p.colorClass} flex items-center justify-center overflow-hidden`}>
                  <div className="text-center px-4">
                    <div className="mag-card-head text-gray-700 dark:text-ink-200 dark:opacity-90">{p.name}</div>
      ...
      <section className="relative mag-section mag-bg-paper py-12 lg:py-24 overflow-hidden">
        <div className="aurora opacity-50" />
        <div className="reveal relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <p className="mag-section-index">09 · Kết nối</p>
          <h2 className="mag-section-head !mt-4">
            Đã đọc hết hồ sơ năng lực?
          </h2>
          <p className="mag-body-lead mt-5">
